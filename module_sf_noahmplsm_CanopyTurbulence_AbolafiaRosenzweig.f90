...
MODULE MODULE_SF_NOAHMPLSM
  ...
  use module_shr_file, only : shr_file_getUnit, shr_file_freeUnit
...
! **recommended
  ...
  INTEGER :: OPT_SFC  ! options for surface layer drag coeff (CH & CM)
              ...
              ! **5 -> MO with RSL parameterization (Harman and Finnigan, 2007&2008; Bonan et al., 2018)
...
!------------------------------------------------------------------------------------------!
! From psihat table (psihatH.dat and psihatM.dat)
!------------------------------------------------------------------------------------------!
REAL,   DIMENSION(1,41)   :: dtLgridM       ! Grid of dtL on which psihat is given for momentum
REAL,   DIMENSION(276,1)  :: zdtgridM      ! Grid of zdt on which psihat is given for momentum
REAL,   DIMENSION(276,41)   :: psigridM     ! Grid of psihat values for momentum
REAL,   DIMENSION(1,41) :: dtLgridH     ! Grid of dtL on which psihat is given for heat
REAL,   DIMENSION(276,1)   :: zdtgridH     ! Grid of zdt on which psihat is given for heat
REAL,   DIMENSION(276,41)  :: psigridH     ! Grid of psihat values for heat
...
!  REAL                           , INTENT(INOUT) :: QRAIN  !rain at ground srf (mm/s) [+]
...
!Added by Ronnie
REAL :: deltaWGDIF
!Outputs added by Ronnie
REAL, INTENT(OUT) ::  RB           !bulk leaf boundary layer resistance (s/m)
REAL, INTENT(OUT) ::  RAMC         !aerodynamic resistance for momentum (s/m)
...
! total water storage for water balance check
     ...
     DO IZ = 1,NSOIL
        ...
        !print*, "SMC-BEG:", SMC(IZ) * DZSNSO(IZ) * 1000
     END DO
     END IF
!print*, "CANLIQ-BEG:", CANLIQ
!print*, "CANICE-BEG:", CANICE
!print*, "WA-BEG:", WA
!print*, "BEG_WB:", BEG_WB
...
! water and energy balance check
     ...
     CALL ERROR (parameters,SWDOWN ,FSA    ,FSR    ,FIRA   ,FSH    ,FCEV , deltaWGDIF, & !in
...
!== begin error ====================================================================================
  ...
  SUBROUTINE ERROR (parameters,SWDOWN ,FSA    ,FSR    ,FIRA   ,FSH    ,FCEV  , deltaWGDIF, &
...
!deltaWGDIF Added by Ronnie:
REAL                           , INTENT(IN) :: deltaWGDIF
...
DO IZ = 1,NSOIL
  print*, "SMC:", SMC(IZ) * DZSNSO(IZ) * 1000
END DO
           if (ERRWAT > 0) then
...
!Outputs added by Ronnie
REAL, INTENT(INOUT) ::  RB           !bulk leaf boundary layer resistance (s/m)
REAL, INTENT(OUT) ::  RAMC         !aerodynamic resistance for momentum (s/m)
REAL, INTENT(OUT) ::  RAHC         !aerodynamic resistance for sensible heat (s/m)
REAL, INTENT(OUT) ::  RAWC         !aerodynamic resistance for water vapor (s/m)
REAL, INTENT(OUT) ::  RAHG         !aerodynamic resistance for sensible heat (s/m)
...
!jref - NITERC test from 5 to 20  
  INTEGER, PARAMETER :: NITERC = 20   !NOTE: Bonan et al. (2018) uses 40 -Ronnie
!jref - NITERG test from 3-5
  ...
  INTEGER :: MLITER     !Last iteration for OBUFUNC
...
!initialize psihat grid for momentum and heat:
!call LookupPsihatINI(dtLgridM,zdtgridM,psigridM,dtLgridH,zdtgridH,psigridH)
dtLgridM=parameters%dtLgridM
zdtgridM=parameters%zdtgridM
psigridM=parameters%psigridM
dtLgridH=parameters%dtLgridH
zdtgridH=parameters%zdtgridH
psigridH=parameters%psigridH
...
IF (OPT_SFC == 5 .AND. HCAN >= 1) THEN
loop2: DO MITER = 1, NITERC    !  begin stability iteration
        IF(MITER == 1) THEN
            MOL = 0.1
            MOZG = 0
        ENDIF
        CALL OBUFUNC(parameters,ILOC, JLOC, HG, TAH, ZPD, &                   !in
        H, ITER, VAI, RHOAIR, TV, HCAN, Z0H, &               !in
...
IF (MLITER == 1) THEN
      exit loop2
   ENDIF
   IF (ABS(MOLdif) <= 0.1) THEN
      MLITER = 1
   ENDIF

END DO loop2 ! end stability iteration

        CALL aeroConductance(parameters,ILOC, JLOC, ZPD, & !in
        MOL, ITER, VAI, HCAN, Z0H, Z0M, & !in
...
! 2m temperature over vegetation ( corrected for low CQ2V values )
   IF (OPT_SFC == 1 .OR. OPT_SFC == 2 .OR. OPT_SFC == 5) THEN
...
! -----------------------------------------------------------------
      loop4: DO ITER = 1, NITERB  ! begin stability iteration
...
IF(OPT_SFC == 1 .or. OPT_SFC==5) THEN
          CALL SFCDIF1(parameters,ITER   ,SFCTMP ,RHOAIR ,H      ,QAIR   , & !in
                       ZLVL   ,ZPD    ,Z0M    ,Z0H    ,UR     , & !in
                       MPE    ,ILOC   ,JLOC   ,                 & !in
                       MOZ    ,MOZSGN ,FM     ,FH     ,FM2,FH2, & !inout
...
! for M-O length
     ...
     END DO loop4 ! end stability iteration
! -----------------------------------------------------------------
...
! leaf boundary layer resistance
       ...
       RB     = MIN(MAX(RB, 5.0),50.0) ! limit RB to 5~50
!       RB     = MAX(RB,100.0)  ! typically RB<50,so this leads to constant RB of 100
!       RB = 200
...
! in & out
    ...
    REAL,              INTENT(INOUT) :: FH2    !Monin-Obukhov heat adjustment at 2m
...
!the following implementation follow Bonan et al. (2018)
!Bonan et al. (2018) code: (https://github.com/gbonan/CLM-ml_v0/blob/master/CanopyTurbulenceMod.F90)
!!=====================================================================================================
  SUBROUTINE OBUFUNC(parameters,ILOC, JLOC, HG, TAH, ZPD, &                   !in
                    H, ITER, VAI, RHOAIR, TV, HCAN, Z0H, &               !in
                    MPE,ZLVL, QSFC, QAIR, SFCTMP, UR, &              !in
                    dtLgridM, zdtgridM, psigridM, dtLgridH, psigridH,zdtgridH, &    !in
                    MOL, MOLdif, FHG, MOZG, MOZSGN, &                              !inout
...
! inputs
    ...
    REAL,                   INTENT(IN) :: MPE                     !prevents overflow error if division by zero
    REAL,                   INTENT(IN) :: ZLVL                    !reference height  (m)
    REAL,                   INTENT(IN) :: QSFC                    !Specific humidity at canopy top (kg/kg)
    REAL,                   INTENT(IN) :: QAIR                    !Specific humidity at height zlvl (kg/kg)
    REAL,                   INTENT(IN) :: SFCTMP                  !canopy air temperature
    REAL,                   INTENT(IN) :: UR                      !wind speed (m/s)
    
    REAL,   DIMENSION(1,41),  INTENT(IN) :: dtLgridM              ! Grid of dtL on which psihat is given for momentum
    REAL,   DIMENSION(276,1), INTENT(IN) :: zdtgridM              ! Grid of zdt on which psihat is given for momentum
    REAL,   DIMENSION(276,41),INTENT(IN) :: psigridM              ! Grid of psihat values for momentum
    REAL,   DIMENSION(1,41),  INTENT(IN) :: dtLgridH              ! Grid of dtL on which psihat is given for heat
    REAL,   DIMENSION(276,1), INTENT(IN) :: zdtgridH              ! Grid of zdt on which psihat is given for heat
    REAL,   DIMENSION(276,41),INTENT(IN) :: psigridH              ! Grid of psihat values for heat

!inout
    REAL,                   INTENT(INOUT) :: MOL                  ! Obukhov length (m)
    REAL,                   INTENT(INOUT) :: MOLdif              ! Difference in Obukhov length between current and previous iteration (m)
    REAL,                   INTENT(INOUT) :: FHG                  !sen heat stability correction, ground
    REAL,                   INTENT(INOUT) :: MOZG                 !Monin-Obukhov stability parameter
    INTEGER,                INTENT(INOUT) :: MOZSGN               !number of times moz changes sign
...
!outputs
    ...
    REAL,                   INTENT(OUT) :: MOZ2                   !2/L

!locals
    REAL    :: Z0MG                           !roughness length, momentum, ground (m)
    REAL    :: MOLG
    REAL    :: FHGNEW
    REAL    :: c1                             ! RSL magnitude multiplier
    REAL    :: BETAneutral                   ! Neutral value for beta = u*/u(h), ~0.3-0.5
    REAL    :: AA                             ! Terms for quadratic or cubic solution
    REAL    :: BB5                            ! Term for quadratic or cubic solution
    REAL    :: CC                             ! Term for quadratic or cubic solution
    REAL    :: DD                             ! Term for quadratic or cubic solution
    REAL    :: QQ                             ! Term for quadratic or cubic solution
    REAL    :: RR                             ! Term for quadratic or cubic solution
    ...
    REAL    :: tvstar                         ! Virtual potential temperature scale (K)
    REAL    :: MOLval                        ! Input value for Obukhov length (m)
    REAL    :: TVIR                           !virtual potential air temp
    REAL    :: MOZ                            !Monin-Obukhov stability (z/L)
    REAL    :: TMP1                           !temporary calculation
    REAL    :: MOZOLD                         !Monin-Obukhov stability parameter from prior iteration

!Parameters
    REAL,       PARAMETER :: Prn = 0.5                ! Neutral value for Pr
    REAL,       PARAMETER :: Prvr = 0.3               ! Magnitude of variation of Pr with stability
    INTEGER,    PARAMETER :: Prsc = 2                 ! Scale of variation of Pr with stability
    REAL,       PARAMETER :: cd = 0.25                ! RSL leaf drag coefficient (dimensionless)
    REAL,       PARAMETER :: BETAneutralmax = 0.35  ! RSL maximum value for beta in neutral conditions
    REAL,       PARAMETER :: cr = 0.3                 ! RSL parameter to calculate BETAneutral
    REAL,       PARAMETER :: Scn = Prn                ! Neutral value for Sc
    REAL,       PARAMETER :: Scvr = Prvr              ! Magnitude of variation of Sc with stability
    REAL,       PARAMETER :: Scsc = Prsc              ! Scale of variation of Sc with stability
    REAL,       PARAMETER :: VKC = 0.40               ! RSL parameter to calculate BETAneutral
    REAL,       PARAMETER :: GRAV   = 9.80616   !acceleration due to gravity (m/s2)
  
    
! --------------------------------------------------------------------------------------------------

!Define Z0MG as done in Bonan et al. (2018)
Z0MG = 0.01
...
ENDIF
MOLval=MOL
!The solution for the Monin-Obukhov length requires an iterative numerical calculation. A value for beta is obtain for an initial estimate of MOL
! using Eq. A22 from Bonan et al., 2018
!Determine neutral value of beta
!Bonan et al., 2018 notes Beta-neutral can be set to 0.35. However, this equation follows from line 759 and 760 of the CLM implementation.
c1 = (VKC / log( (HCAN + Z0MG) / Z0MG) )**2
BETAneutral = min( sqrt(c1 + cr*(VAI)), BETAneutralmax)

!Calculate beta = u*/u(h) for current Obukhov length

!Lc does not change in iterations, Eq. A27 of Bonan et al. (2018) and line 380 of code
Lc = HCAN/ (cd * VAI)
LcL = Lc/MOL

LcLval=LcL
IF (LcLval <=0) THEN
    !Unstable case: quadratic equation for beta^2 at LcLval (Bonan et al., 2018 eq. A23)
    BB5= 16 * LcLval * BETAneutral**4
    BETAMOL= sqrt( 0.5*(-BB5 + sqrt(BB5**2 + 4 * BETAneutral**4)) )
ELSE
    !Stable case: cubic equation for beta at LcLval (Bonan et al., 2018 eq. A24)
    AA = 5*LcLval
    BB5 = 0
    CC = 1
    DD = -BETAneutral
    QQ = (2*BB5**3 - 9*AA*BB5*CC + 27*(AA**2)*DD)**2 - 4*(BB5**2 - 3*AA*CC)**3
    QQ = sqrt(QQ)
    RR = 0.5 * (QQ + 2*BB5**3 - 9*AA*BB5*CC + 27*(AA**2)*DD)
    RR = RR**(1/3)
    BETAMOL = -(BB5+RR)/(3*AA) - (BB5**2 - 3*AA*CC)/(3*AA*RR)
ENDIF
!Constrain Beta (Bonan et al., 2018 - text between A24 and A25)
BETAMOL = min(0.50, max(BETAMOL,0.20))
!solve for mixing length of momentum (lm; Eq. A26 of Bonan et al. 2018):
lm = 2 * BETAMOL**3 * Lc
!Displacement height: dt is height below canopy top (dt = HCAN - zdisp)
!From lines 771-774 of Bonan's code and Eq. A28 of Bonan et al. (2018)
dtMOL= BETAMOL**2 * Lc
dtMOL = dtMOL * (1 - exp( (-0.25*VAI) / BETAMOL**2) )
dtMOL = min(HCAN, dtMOL)
zdisp = HCAN - dtMOL
IF (ZLVL - zdisp < 0) THEN
    CALL wrf_error_fatal (' ERROR: OBUFUNC: error, zdisp height > ZLVL')
ENDIF
...
!The Schmidt and Prandt numbers are parameterized by Harman and Finnigan (2008) and presented in Eq. A25 of Bonan et al. (2018)
Pr = Prn + Prvr * tanh(Prsc*LcL)
Pr = (1 - BETAneutral/BETAneutralmax) + BETAneutral/BETAneutralmax*Pr
    
Sc = Scn + Scvr * tanh(Scsc*LcL)
Sc = (1 - BETAneutral/BETAneutralmax) + BETAneutral/BETAneutralmax*Sc
...
!This calculation follows from Bonan et al. (2018) text directly before Eq. A12
!Constrain Zeta between -2 to 1, following Foken (2006) and Bonan et al. (2018)
zeta = (ZLVL - zdisp) / MOL
IF (zeta > 1) THEN
    zeta = 1
ELSEIF (zeta < -2) THEN
...
!Solve for psim and psic:
call GetPsiRSL(dtLgridM,zdtgridM,psigridM,dtLgridH,zdtgridH,psigridH,ZLVL, HCAN, dtMOL, BETAMOL, zeta, MOL, PrSc, psim, psic)

! Friction velocity
zlog = log( (ZLVL - zdisp) / (HCAN - zdisp) )
FV = UR * VKC / (zlog + psim)

!Wind speed at canopy height
UC = FV / BETAMOL

!for numerical reason zlog+psic ~= 0
IF (zlog+psic==0)THEN
    psic=psic-0.1
ENDIF
!for numerical reason zlog+psic ~= 0
...
!Temperature scale - line 818 of Bonan code
TVIR = (1. + 0.61*QAIR) * SFCTMP
tstar = (TVIR - TAH) * VKC / (zlog+psic)

!water vapor scale - use units of specific humidity (kg/kg)
qstar = (QAIR - QSFC)*VKC / (zlog+psic)
!calculate new Obukhov length (m) - Eq. A29 from Bonan et al. (2018) and line 832
tvstar = tstar + 0.61 * TVIR * qstar
MOL = FV**2 * TVIR / (VKC * GRAV *tvstar)
IF (abs(MOL)<=0.1) THEN
    MOL=0.1
ENDIF

!Change in Obukhov length (m) - this can be used if we would like to change the code to a convergence criteria rather than a set number of iterations
MOLdif = MOL - MOLval

!The RB equivalent in Bonan's code is gbh, which is not calculated in the Canopy turbulence model.
!Therefore, I will maintain the original Noah-MP calculations of RB. - Ronnie
IF(ITER > 1) THEN
 TMP1 = VKC * (GRAV/TAH) * HG/(RHOAIR*CPAIR)
 IF (ABS(TMP1) .LE. MPE) TMP1 = MPE
 MOLG = -1. * FV**3 / TMP1
 MOZG = MIN( (zdisp-Z0MG)/MOLG, 1.)
ENDIF
...
!== begin aeroConductance ====================================================================================
!!====================================================================================================
!Added by Ronnie Abolafia-Rosenzweig
!the following implementation follows Bonan et al. (2018)
!Bonan et al. (2018) code: (https://github.com/gbonan/CLM-ml_v0/blob/master/CanopyTurbulenceMod.F90)
!!=====================================================================================================
  SUBROUTINE aeroConductance(parameters,ILOC, JLOC, ZPD, & !in
                                MOL, ITER, VAI, HCAN, Z0H, Z0M, & !in
                                CWP, FHG, MPE,ZLVL, UR, dtMOL, FV, UC,MOZ, Sc,MOZ2, zdisp, & !in
                                dtLgridM, zdtgridM, psigridM, dtLgridH, zdtgridH, psigridH, & !in
                                FM,FH,FM2,FH2, & !inout
...
! inputs
    ...
    INTEGER,              INTENT(IN) :: JLOC   !grid index
    REAL,                 INTENT(IN) :: ZPD    !zero plane displacement (m)
    REAL,                 INTENT(IN) :: MOL    ! Obukhov length (m)
    INTEGER,              INTENT(IN) :: ITER   !iteration index
    REAL,                 INTENT(IN) :: VAI    !total LAI + stem area index, one sided
    REAL,                 INTENT(IN) :: HCAN   !canopy height (m) [note: hcan >= z0mg]
    REAL,                 INTENT(IN) :: Z0H    !roughness length, sensible heat (m)
    ...
    REAL,                 INTENT(IN) :: ZLVL   !reference height  (m)
    REAL,                 INTENT(IN) :: UR     !wind speed (m/s)
    REAL,                 INTENT(IN) :: dtMOL ! Height below canopy top (dtMOL = HCAN - zdisp)
    REAL,                 INTENT(IN) :: FV     !friction velocity (m/s)
    REAL,                 INTENT(IN) :: UC     !wind speed at top of canopy (m/s)
    REAL,                 INTENT(IN) :: MOZ    !Monin-Obukhov stability (z/L)
    REAL,                 INTENT(IN) :: Sc     !Schmidt number
    REAL,                 INTENT(IN) :: MOZ2   !Monin-Obukhov stability (2/L)
    REAL,                 INTENT(IN) :: zdisp   !Monin-Obukhov stability (2/L)
...
! outputs
    ...
    REAL, INTENT(OUT) :: RAHC                 !aerodynamic resistance for sensible heat (s/m) - above canopy (z>HCAN)
    REAL, INTENT(OUT) :: RAWC                 !aerodynamic resistance for water vapor (s/m) - above canopy (z>HCAN)
    REAL, INTENT(OUT) :: FH2                  !sen heat stability correction, weighted by prior iters
    REAL, INTENT(OUT) :: KH                         ! Kc at HCAN - solved from Eq 23 of Bonan et al. (2018)
...
! locals
    ...
    REAL :: Lc                             ! Canopy density length scale (m)
    REAL :: zeta                           ! Monin-Obukhov stability parameter (z-d)/L
    REAL :: psim1                          ! psi function for momentum
    REAL :: psic                           ! psi function for scalar
    REAL :: psic1                          ! psi function for scalar
    REAL :: psim2                          ! psi function for momentum
    ...
    REAL :: beta_over_lm                   ! beta/lm (1/m)
    REAL :: eta                            ! temporary calc for beta_over_lm
    REAL :: RSL_top                        !Top of RSL (2X HCAN; Bonan et al., 2018)
    ...
    REAL :: zlogc                          ! scalar log profile at ground
    REAL :: Z0MG                           !roughness length, momentum, ground (m)
    REAL :: FMNEW                          !stability correction factor, momentum, for current moz
    REAL :: FHNEW                          !stability correction factor, sen heat, for current moz
    ...
    REAL :: TMPCM                          !temporary calculation for CM
    REAL :: TMPCH                          !temporary calculation for CH
    REAL :: TMPRB                          !temporary calculation for CH
! --------------------------------------------------------------------------------------------------

!!========================================
!Added by Ronnie Abolafia-Rosenzweig
!the following implementation follow Bonan et al. (2018)
!Bonan code can be found here: https://github.com/gbonan/CLM-ml_v0/blob/master/CanopyTurbulenceMod.F90
!!========================================
BETAMOL = FV/UC
BETAMOL = min(0.50, max(BETAMOL,0.20))

!Aerodynamic resistance for momentum above canopy: (Bonan code line 2218)
RAMC = UR / (FV*FV)
if (RAMC > 500) then
    RAMC = 500
ENDIF
!Following Bonan et al. (1996 and 2018) use 1 as lower constraint: (for numerical reasons)
if (RAMC < 1) then
    RAMC = 1
ENDIF

!Choosing this method of calc - rather than mean of profile - because extremes in the profile dominate mean
!from line 456 of Bonan code - Aerodynamic conductance above canopy:
!Following Bonan et al. (2018) - we integrate the conductance for z>HCAN, then take the inverse solving for the resistance
RSL_top=ZLVL
RSL_bottom=HCAN
zeta = (RSL_bottom - zdisp) / MOL
call GetPsiRSL(dtLgridM,zdtgridM,psigridM,dtLgridH,zdtgridH,psigridH,HCAN, HCAN, dtMOL, BETAMOL, zeta, MOL, PrSc, psim1, psic1)
zeta = (RSL_top - zdisp)/MOL
call GetPsiRSL(dtLgridM,zdtgridM,psigridM,dtLgridH,zdtgridH,psigridH,ZLVL, HCAN, dtMOL, BETAMOL, zeta, MOL, PrSc, psim2, psic2)
psic=psic2-psic1
zlogc=log( ( RSL_top-zdisp)/(RSL_bottom-zdisp) )
!for numerical reason zlog+psic ~= 0
IF (zlogc+psic==0)THEN
    psic=psic-0.1
ENDIF
gah = (VKC*FV)/(zlogc+psic)  !resistance s/m
RAHC = 1/gah
!constrain resistances to be below 500 s m-1 (Bonan et al., 2018)
RAHC = min(RAHC,500)
RAHC = max(RAHC,1)
RAWC = RAHC
!The RB equivalent in Bonan's code is gbh, which is not calculated in the Canopy turbulence model.
...
!Calculate RAHG -
!rah,g aerodynamic resistance below the canopy for heat [Niu and Yang, 2004 & 2011];
!Bonan code line 531

!using eq. 26 from Bonan et al. (2018) - calculate RAHG within the canopy
...
beta_over_lm = eta/HCAN
KH = 2 * BETAMOL**3 * Lc * Fv / Sc !Eq. 23 of Bonan et al. (2018)

Z1LVL = 0.05
zincr=(zdisp+Z0H-Z1LVL)/19
rahgCUM = 0 ! intialize cumulative gah
do kk = 1,20
    if (kk==1) then
        z_tmp=Z1LVL
    else
    ...
    RAHGveg = 1 / (KH * beta_over_lm) * ( exp( -(z_tmp-HCAN) *beta_over_lm )  - exp( -(z_tmp+zincr-HCAN)*beta_over_lm ) )
    RAHGveg = min(RAHGveg,200)
    rahgCUM = rahgCUM + RAHGveg
end do

!using eq. 27 from Bonan et al. (2018) - calculate RAHG at the ground
WindG = UC * exp( (Z1LVL - HCAN) * beta_over_lm )
WindG=max(WindG,0.1)
zlogm = log(Z1LVL/Z0MG)
zlogc = log(Z1LVL/Z0HG)
FVg = WindG * vkc / zlogm
FVg = max(FVg,0.01)
RAHGground = zlogc / (vkc * FVg)
RAHGground = min(RAHGground,200)
!Calculate cumulative RAHG within canopy and directly above the ground:
RAHG = rahgCUM + RAHGground
!RAHG = min(RAHG,500)
RAHG = max(RAHG,1)
RAWG = RAHG
...
! exchange coefficients
    TMPCM = LOG((ZLVL-zdisp) / Z0M)
    TMPCH = LOG((ZLVL-zdisp) / Z0H)
    TMPCM2 = LOG((2.0 + Z0M) / Z0M)
    TMPCH2 = LOG((2.0 + Z0H) / Z0H)
    FH = MIN(FH,0.9*TMPCH)
    FM = MIN(FM,0.9*TMPCM)
...
END SUBROUTINE aeroConductance

!subroutine GetPsiRSL is directly from Bonan et al. (2018) code - added to Noah-MP by Ronnie Abolafia-Rosenzweig
!== begin GetPsiRSL ==================================================================================
subroutine GetPsiRSL (dtLgridM,zdtgridM,psigridM,dtLgridH,zdtgridH,psigridH,ZLVL, HCAN, dtMOL, BETAMOL, zeta, MOL, PrSc, psim, psic)
  !
  ! !DESCRIPTION:
  ! Calculate the stability functions psi for momentum and scalars. The
  ! returned function values (psim, psic) are the Monin-Obukhov psi functions
  ...
  !INPUTS
  REAL, intent(in)      :: ZLVL        !reference height  (m)
  REAL, intent(in)      :: HCAN        ! Canopy height (m)
  REAL, intent(in)      :: dtMOL      ! Height below canopy top (dt = ztop - zdisp)
  REAL, intent(in)      :: BETAMOL    ! Value of u*/u(h) at canopy top
  REAL, intent(in)      :: zeta        ! Monin-Obukhov stability parameter (za-d)/L
  REAL, intent(in)      :: MOL         ! Obukhov length (m)
  INTEGER, intent(in)   :: PrSc        ! Turbulent Prandtl (Schmidt) number at canopy top
  ...
  ! !LOCAL VARIABLES:
  REAL :: phim                   ! Monin-Obukhov phi function for momentum at canopy top
  REAL :: phic                   ! Monin-Obukhov phi function for scalars at canopy top
  REAL :: c1                     ! RSL magnitude multiplier
  REAL :: psihat1                ! Multiply by c1 to get the RSL psihat function evaluated at za
  REAL :: psihat2                ! Multiply by c1 to get the RSL psihat function evaluated at hc

  REAL, parameter :: c2 = 0.5    ! RSL depth scale multiplier
  REAL, parameter :: VKC = 0.40  ! von Karman constant
  !OUTPUTS
  REAL, intent(out)     :: psim        ! psi function for momentum including RSL influence
  REAL, intent(out)     :: psic        ! psi function for scalars  including RSL influence
  !---------------------------------------------------------------------
  ...
  !Retrieve psihat values from lookup table (Bonan et al., 2018).
  !Note that the lookup tables/grids have been stored as Noah-MP parameters similar to LAIM
  call LookupPsihat ((ZLVL-HCAN)/dtMOL, dtMOL/MOL, zdtgridM, dtLgridM, psigridM, psihat1)
  call LookupPsihat ((HCAN-HCAN)/dtMOL, dtMOL/MOL, zdtgridM, dtLgridM, psigridM, psihat2)

  !This function is directly from Bonan et al. (2018) code
  phim = phim_monin_obukhov(dtMOL/MOL)
  !Eq. A16 from Bonan et al. (2018)
  c1 = (1 - VKC / (2 * BETAMOL * phim)) * exp(0.5*c2)
  psim = -psim_monin_obukhov(zeta) + psim_monin_obukhov(dtMOL/MOL) + c1*(psihat1 - psihat2) + VKC / BETAMOL

  ! Now do the same for heat
  call LookupPsihat ((ZLVL-HCAN)/dtMOL, dtMOL/MOL, zdtgridH, dtLgridH, psigridH, psihat1)
  call LookupPsihat ((HCAN-HCAN)/dtMOL, dtMOL/MOL, zdtgridH, dtLgridH, psigridH, psihat2)
  phic = phic_monin_obukhov(dtMOL/MOL)
  !Eq A19 from Bonan et al. (2018)
  c1 = (1 - PrSc*VKC / (2 * BETAMOL * phic)) * exp(0.5*c2)
  psic = -psic_monin_obukhov(zeta) + psic_monin_obukhov(dtMOL/MOL) + c1*(psihat1 - psihat2)

end subroutine GetPsiRSL

!subroutine LookupPsihat is directly from Bonan et al. (2018) code - Ronnie Abolafia-Rosenzweig
!== begin LookupPsihat ==================================================================================
 subroutine LookupPsihat (zdt, dtL, zdtgrid, dtLgrid, psigrid, psihat)
   ...
   ! !ARGUMENTS:
   implicit none
   REAL, intent(in) :: zdt            ! Height (above canopy) normalized by dt
   REAL, intent(in) :: dtL            ! dt/L (displacement height/Obukhov length)

   REAL, DIMENSION(276,1), intent(in)  :: zdtgrid  ! Grid of zdt on which psihat is given
   REAL, DIMENSION(1,41), intent(in)   :: dtLgrid  ! Grid of dtL on which psihat is given
   REAL, DIMENSION(276,41), intent(in) :: psigrid  ! Grid of psihat values
   REAL, intent(out):: psihat                      ! Value of psihat
   !
   !LOCAL VARIABLES
   integer  :: ii, jj                     ! Looping indices
   integer  :: L1, L2, Z1, Z2             ! Grid indices for psihat sought
   REAL     :: wL1, wL2, wZ1, wZ2         ! Weights for averaging
   integer  :: nZ                         !dimension of grid (nrows)
   integer  :: nL                         !dimension of grid (ncols)
   !---------------------------------------------------------------------
   nZ=276
   nL=41
   ...
   if (L1 == 0 .or. L2 == 0) then
        print*,"dtL",dtL
print*,"dtLgrid(1,1)",dtLgrid(1,1)
print*,"dtLgrid(1,nL) ",dtLgrid(1,nL)
      call wrf_error_fatal (' ERROR: LookupPsihat error, indices L1 and L2 not found')
   end if
   ...
   if (Z1 == 0 .or. Z2 == 0) then
      call wrf_error_fatal (' ERROR: LookupPsihat error, indices Z1 and Z2 not found')
   end if
 ...
 subroutine LookupPsihatINI(dtLgridM,zdtgridM,psigridM,dtLgridH,zdtgridH,psigridH) !out
    !
    ! !DESCRIPTION:
    ...
    integer :: ii, jj                  ! Looping indices
    integer :: nZ                   ! Monin-Obukhov phi function for momentum at canopy top
    integer :: nL                   ! Monin-Obukhov phi function for scalars at canopy top
    !OUTPUTS
    REAL,   DIMENSION(1,41), intent(OUT)    :: dtLgridM       ! Grid of dtL on which psihat is given for momentum
    REAL,   DIMENSION(276,1), intent(OUT)    :: zdtgridM      ! Grid of zdt on which psihat is given for momentum
    REAL,   DIMENSION(276,41), intent(OUT)    :: psigridM     ! Grid of psihat values for momentum
    REAL,   DIMENSION(1,41),intent(OUT)    :: dtLgridH     ! Grid of dtL on which psihat is given for heat
    REAL,   DIMENSION(276,1),intent(OUT)    :: zdtgridH     ! Grid of zdt on which psihat is given for heat
    REAL,   DIMENSION(276,41), intent(OUT)    :: psigridH     ! Grid of psihat values for heat
!---------------------------------------------------------------------
    
    !Define dimensions of look up table
    nZ=276
    nL=41
...
10  continue
    call wrf_error_fatal (' ERROR: LookupPsihatINI error opening psihatM.dat')

11  continue
    call wrf_error_fatal (' ERROR: LookupPsihatINI error reading dtLgridM')

12  continue
    call wrf_error_fatal (' ERROR: LookupPsihatINI error reading psigridM')

20  continue
    call wrf_error_fatal (' ERROR: LookupPsihatINI error opening psihatH.dat')

21  continue
    call wrf_error_fatal (' ERROR: LookupPsihatINI error reading dtLgridH')

22  continue
    call wrf_error_fatal (' ERROR: LookupPsihatINI error reading psigridH')

  end subroutine LookupPsihatINI


!function phim_monin_obukhov is directly from Bonan et al. (2018) code - Ronnie Abolafia-Rosenzweig
!== begin phim_monin_obukhov ==================================================================================
function phim_monin_obukhov (zeta) result(phi)
  ...
  ! !ARGUMENTS:
  implicit none
  REAL, intent(in) :: zeta  ! Monin-Obukhov stability parameter
  !
  ! !LOCAL VARIABLES:
  REAL :: phi               ! phi for momentum
  !---------------------------------------------------------------------

  if (zeta < 0.) then           ! --- unstable
     phi = 1. / sqrt(sqrt(1. - 16. * zeta))
  else                          ! --- stable
     phi = 1. + 5. * zeta
  ENDIF
...
!function phic_monin_obukhov is directly from Bonan et al. (2018) code - Ronnie Abolafia-Rosenzweig
!== begin phic_monin_obukhov ==================================================================================
...
function StabilityFunc2 (zeta) result(psi)
  !
  ! !DESCRIPTION:
  ! Noah-MP Monin-Obukhov stability function for scalars (from CLM; Bonan et al. 2018)
  !
  ! !ARGUMENTS:
  implicit none
  REAL, intent(in) :: zeta  ! Monin-Obukhov stability parameter (z-d)/L
  !
  ! !LOCAL VARIABLES:
  REAL :: x                 ! (1 - 16*zeta)**1/2
  REAL :: psi               ! psi for scalars
  !---------------------------------------------------------------------
...
function psim_monin_obukhov (zeta) result(psi)
  ...
  !
  ! !LOCAL VARIABLES:
  REAL :: x                 ! (1 - 16*zeta)**1/4
  REAL :: psi               ! psi for momentum
  !---------------------------------------------------------------------

  if (zeta < 0) then           ! --- unstable
     x = sqrt(sqrt(1 - 16 * zeta))
     psi = 2 * log((1+x)/2) + log((1+x*x)/2) - 2*atan(x) + 3.14159265 * 0.5
  else                             ! --- stable
     psi = -5 * zeta
  end if
...
! sublimation, frost, evaporation, and dew
     ...
     CALL SNOWWATER (parameters,NSNOW  ,NSOIL  ,IMELT  ,DT     ,ZSOIL  , & !in
          ...
          &          QSNBOT ,SNOFLOW,PONDING1       ,PONDING2, deltawgdif)  !out
...
! MB: do each if block separately
   ...
   CALL  SNOWH2O (parameters,NSNOW  ,NSOIL  ,DT     ,QSNFRO ,QSNSUB , & !in 
                  ...
                  QSNBOT ,PONDING1       ,PONDING2, deltawgdif)           !out
...
!Ronnie changed WGDIF to an output
   REAL                      :: WGDIF     !ice mass after minus sublimation
   REAL                      :: WGDIFtmp     !temporary calcualtion for change in ice mass after minus sublimation
   REAL, INTENT(OUT)                        :: deltaWGDIF     !change ice mass after minus sublimation
...
! for deep snow
   ...
   IF ( ISNOW < 0 ) THEN !KWM added this IF statement to prevent out-of-bounds array references
      !This can trigger ERRWAT if WGDIFtmp is negative (leading to negative SNICE) and ISNOW is negative
...
!OUTPUTS
REAL   :: dtLgridM(1,41)       ! Grid of dtL on which psihat is given for momentum
REAL  :: zdtgridM(276,1)     ! Grid of zdt on which psihat is given for momentum
REAL    :: psigridM(276,41)     ! Grid of psihat values for momentum
REAL   :: dtLgridH(1,41)     ! Grid of dtL on which psihat is given for heat
REAL  :: zdtgridH(276,1)     ! Grid of zdt on which psihat is given for heat
REAL    :: psigridH(276,41)     ! Grid of psihat values for heat
...
CONTAINS
  ...
  subroutine read_mp_veg_parameters(DATASET_IDENTIFIER)
    ...
    nin = shr_file_getUnit()
    open (unit=nin, file=trim(fin), action="read", status="old", err=10)
    read (nin,*,err=11) dtLgridM(1,1),(dtLgridM(1,jj), jj = 1,41)
    do ii = 1, 276
      read (nin,*,err=12) zdtgridM(ii,1),(psigridM(ii,jj), jj = 1,41)
    end do
    close (nin)
    call shr_file_freeUnit (nin)
    ...
    nin = shr_file_getUnit()
    open (unit=nin, file=trim(fin), action="read", status="old", err=20)
    read (nin,*,err=21) dtLgridH(1,1),(dtLgridH(1,jj), jj = 1,41)
    do ii = 1,276
      read (nin,*,err=22) zdtgridH(ii,1),(psigridH(ii,jj), jj = 1,41)
    end do
    close (nin)
    call shr_file_freeUnit (nin)
